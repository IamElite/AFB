...
import logging
from database.ia_filterdb import src_get_movies, src_get_series
...
async def src_list_movies(client, message):
    try:
        movies = await src_get_movies()
        if not movies:
        ...
        logger.error(f"Error in src_list_movies: {e}")
...
async def src_list_series(client, message):
    try:
        series_data = await src_get_series()
        if not series_data:
        ...
        logger.error(f"Error in src_list_series: {e}")
        await message.reply("An Error Occurred ☹️", parse_mode=ParseMode.HTML)
