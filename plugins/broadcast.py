...
async def broadcast_users(bot, message):
    ...
    try:
        src_user_response = await bot.listen(chat_id=message.chat.id, user_id=message.from_user.id, timeout=60)
    except asyncio.TimeoutError:
        await ask.delete()
        return await message.reply("❌ Timed out. Broadcast cancelled.")
    await ask.delete()
    if src_user_response.text not in ("Yes", "No"):
        return await message.reply("❌ Invalid input. Broadcast cancelled.")

    is_pin = src_user_response.text == "Yes"
    b_msg = message.reply_to_message
    users = [user async for user in await db.get_all_users()]
    total_users = len(users)
    src_status_msg = await message.reply_text("📤 <b>Broadcasting your message...</b>")
    ...
    await src_status_msg.edit(final_status)
...
async def broadcast_group(bot, message):
    ...
    src_status_msg = await message.reply_text("📤 <b>Broadcasting your message to groups...</b>")
    start_time = time.time()
    ...
    try:
        await src_status_msg.edit(src_text)
    except MessageTooLong:
        with open("reason.txt", "w+") as outfile:
            outfile.write(str(failed))
        await message.reply_document(
            "reason.txt", caption=src_text
        )
