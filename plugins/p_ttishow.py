...
async def save_group(bot, message):
    src_check = [u.id for u in message.new_chat_members]
    if temp.ME in src_check:
        if not await db.get_chat(message.chat.id):
            total=await bot.get_chat_members_count(message.chat.id)
            src_botz = message.from_user.mention if message.from_user else "Anonymous" 
            await bot.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total, src_botz))       
...
async def list_users(bot, message):
    src = await message.reply('Getting List Of Users')
...
async def list_chats(bot, message):
    src = await message.reply('Getting List Of chats')
