...
async def next_page(bot, query):
    ...
    if not settings["button"]:
        ...
        src_title = clean_search_text(search)
        cap = None
        try:
            if settings['imdb']:
                cap = await get_cap(settings, remaining_seconds, files, query, total, src_title, offset)
                ...
                cap = await get_cap(settings, remaining_seconds, files, query, total, src_title, offset+1)
                await query.message.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)
        except MessageNotModified:
            pass
        except Exception as e:
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not settings["button"]:
        ...
        remaining_seconds = "{:.2f}".format(time_difference.total_seconds())
        src_title = clean_search_text(search)
        cap = await get_cap(settings, remaining_seconds, files, query, total_results, src_title, offset=1)
        try:
            await query.message.edit_text(text=cap, reply_markup=InlineKeyboardMarkup(btn), disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML)
        except MessageNotModified:
            pass
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not settings.get("button"):
        ...
        src_title = clean_search_text(search_final)
        cap = await get_cap(settings, remaining_seconds, files, query, total_results, src_title, offset=1)
        try:
            await query.message.edit_text(
                text=cap,
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
    ...
    elif srcData.startswith("generate_stream_link"):
        _, file_id = srcData.split(":")
        try:
            ...
            await log_msg.reply_text(
                ...
                quote=True,
                disable_web_page_preview=True,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ 🚀", url=src_download),  # we download Link
                                                    InlineKeyboardButton('🖥️ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 🖥️', url=src_stream)]])  # web stream Link
            )
            dreamcinezone = await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup([
                    [
                        InlineKeyboardButton("🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ 🚀", url=src_download),
                        InlineKeyboardButton('🖥️ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ 🖥️', url=src_stream)
                    ],
                    [
        ...
        await query.message.edit_text(
            text=script.src_DONATION.format(query.from_user.mention, QR_CODE, OWNER_UPI_ID),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
            InlineKeyboardButton('ᴅʀᴇᴀᴍxʙᴏᴛᴢ 📜', url='https://github.com/src/Auto_Filter_Bot.git'),
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ⇋', callback_data='about')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        btn = await group_setting_buttons(int(grp_id))
        src = await client.get_chat(int(grp_id))
        await query.message.edit(text=f"ᴄʜᴀɴɢᴇ ʏᴏᴜʀ ɢʀᴏᴜᴘ ꜱᴇᴛᴛɪɴɢꜱ ✅\nɢʀᴏᴜᴘ ɴᴀᴍᴇ - '{src.title}'</b>⚙", reply_markup=InlineKeyboardMarkup(btn))
